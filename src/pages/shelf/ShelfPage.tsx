...
export default function ShelfPage() {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 6 }}>
      <Stack spacing={4} alignItems="center">
        ...
        <Box width="100%">
          {loading ? (
            ...
            <Typography color="error">{t("fetchFailed")}: {String(error)}</Typography>
          ) : (
      ...
      <Dialog open={deleteDialog.open} onClose={handleCancelDelete} PaperProps={{ sx: { borderRadius: 3, p: 1, background: "linear-gradient(135deg, #ffffff 0%, #f8fafc 100%)", boxShadow: "0 12px 40px rgba(0,0,0,0.12)", minWidth: 380 } }}>
        <DialogTitle sx={{ display: "flex", alignItems: "center", gap: 1.5, pb: 0 }}>
          <Box sx={{ width: 46, height: 46, borderRadius: "50%", background: "rgba(244,67,54,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <DialogContentText sx={{ fontSize: 15, color: "text.secondary" }}>
            {deleteDialog.name ? t("deleteConfirmBody_single", { name: deleteDialog.name }) : t("deleteConfirmBody_plural")}
          </DialogContentText>
        </DialogContent>

        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={handleCancelDelete} color="inherit" sx={{ borderRadius: 2, textTransform: "none", px: 2.5, "&:hover": { backgroundColor: "rgba(0,0,0,0.04)" } }} disabled={deleting}>
            {t("cancel")}
          </Button>

          <Button onClick={handleConfirmDelete} variant="contained" color="error" disabled={deleting} sx={{ borderRadius: 2, textTransform: "none", px: 3, boxShadow: "0 4px 16px rgba(244,67,54,0.35)" }}>
