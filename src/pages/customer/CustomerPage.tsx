...
export default function CustomerPage() {
  ...
  const fetchFullList = useCallback(async () => {
    try {
      ...
      setSnack({ open: true, message: err?.response?.data?.message || "Failed to load customers", severity: "error" });
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => { fetchFullList(); }, [fetchFullList]);
  ...
  const handleOpenAdd = () => { setEditingCustomer(null); setDialogOpen(true); };
  ...
  const handleSave = async (data: Partial<Customer>) => {
    try {
      ...
      if (editingCustomer) {
        ...
        setSnack({ open: true, message: "Customer created", severity: "success" });
      }
      setDialogOpen(false);
      await fetchFullList();
    } catch (err: any) {
      console.error("save error", err);
      setSnack({ open: true, message: err?.response?.data?.message || "Failed to save customer", severity: "error" });
    } finally { setLoading(false); }
  };
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      ...
      setSnack({ open: true, message: err?.response?.data?.message || "Failed to delete customer", severity: "error" });
    } finally { setLoading(false); }
  };
  ...
  return (
    <Box sx={{ p: { xs: 1.5, sm: 3 } }}>
      ...
      <Card sx={{ mb: 2 }}>
        <CardContent sx={{ p: { xs: 1.25, sm: 2 } }}>
          <CustomerToolbar
            ...
            onSearchTermChange={(v) => { setSearchTerm(v); setPaginationModel((m) => ({ ...m, page: 0 })); }}
            filterStatus={filterStatus}
            onFilterStatusChange={(v) => { setFilterStatus(v); setPaginationModel((m) => ({ ...m, page: 0 })); }}
            onAdd={handleOpenAdd}
          />
        </CardContent>
      </Card>
      ...
      <CustomerDialog open={dialogOpen} title={editingCustomer ? "Edit Customer" : "Add Customer"} onClose={() => setDialogOpen(false)}>
