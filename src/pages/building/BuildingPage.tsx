...
export default function BuildingPage() {
  ...
  const fetchAll = useCallback(
    async (p = 1) => {
      ...
      try {
        ...
        console.error(err);
        setSnack({ open: true, message: t("notify.loadFailed"), severity: "error" });
      } finally {
        setLoading(false);
      }
    },
  ...
  async function onSave(payload: Partial<Building>) {
    try {
      if (editing) {
        ...
        setSnack({ open: true, message: t("notify.updated"), severity: "success" });
        setOpenDialog(false);
        await fetchAll(page);
      } else {
        await api.createBuilding(payload);
        setSnack({ open: true, message: t("notify.created"), severity: "success" });
        setOpenDialog(false);
        setPage(1);
        await fetchAll(1);
      }
      ...
      console.error(err);
      const msg = err?.message || err?.response?.data?.message || t("notify.saveFailed");
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      setSnack({ open: true, message: t("notify.deleted"), severity: "success" });
      ...
      console.error(err);
      const msg = err?.message || err?.response?.data?.message || t("notify.deleteFailed");
      setSnack({ open: true, message: msg, severity: "error" });
    } finally {
      setDeleting(false);
    }
